...
struct ContentCorrect: View {
    var body: some View {
    correctStruct()
    }
}
struct correctStruct : UIViewControllerRepresentable {
...
class correctVC : UIViewController {
    ...
    func setupView () {
        ...
        correctText = UILabel ()
        correctText.numberOfLines = 0
        correctText.textAlignment = .center
        correctText.translatesAutoresizingMaskIntoConstraints = false
        correctText.setTyping(text: """
        
        CORRECT!
        Congratulations you finished this game.
        Tap back to restart the game
        """)
        
        bubleImageView.addSubview(correctText)
        NSLayoutConstraint.activate([
            correctText.topAnchor.constraint(equalTo: bubleImageView.topAnchor, constant: 30),
            correctText.centerXAnchor.constraint(equalTo: bubleImageView.centerXAnchor)
        ])
